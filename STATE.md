...
* CI Fix (lint errcheck): With golangci-lint now actually running (v2.13.2 via action v9), it flagged 5 unchecked `unix.Close` calls in `pkg/probe/fs`. Fixed `observer.go:125-128` (bare calls -> `_ = unix.Close(...)`) and `fanotify.go:111` (`defer unix.Close(fd)` -> `defer func() { _ = unix.Close(fd) }()`), matching the existing suppression style in the same package.
